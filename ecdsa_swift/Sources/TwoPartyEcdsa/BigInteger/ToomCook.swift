...
extension Array where Element == Limb {
    ...
    static func divideBy3(_ d: BigInteger) -> BigInteger {
        ...
        return BigInteger(quotient, d.isNegative)
    }

    func toomCookSlice0(_ k: Int) -> BigInteger {
        let w = k < self.count ? Limbs(self[0 ..< k]) : self
        return BigInteger(w)
    }

    func toomCookSlice1(_ k: Int) -> BigInteger {
        let w = k < self.count ? (2 * k < self.count ? Limbs(self[k ..< 2 * k]) : Limbs(self[k ..< self.count])) : [0]
        return BigInteger(w)
    }

    func toomCookSlice2(_ k: Int) -> BigInteger {
